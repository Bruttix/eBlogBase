...
const Post = ({ ...post }: PostProps) => {
  ...
  return (
    <div
      key={post.id}
      className=" flex flex-col space-y-4 border-b border-yellow-600 pb-8 last:border-none"
    >
      <Link
      ...
      >
        ...
        <div className="postAuthor rounded-2xl">
      ...
      >
        <div className="postContentBox col-span-8 rounded-2xl flex h-full w-full flex-col space-y-4">
          <p className="text-2xl font-bold decoration-indigo-600 group-hover:underline">
            {post.title}
          </p>
          <p className="h-full w-full max-w-sm truncate break-words text-sm">
            {post.description}
          </p>
        </div>
          ...
          <div className="flex items-center space-x-2">
            {post.tags.map((tag) => (
              <div
                ...
                className="rounded-2xl topicTags px-5 py-3"
              >
                {tag.name}
              </div>
            ))}
